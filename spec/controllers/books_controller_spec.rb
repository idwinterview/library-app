...
describe BooksController do
  ...
  context "#check_out" do
    context "when the customer successfully checks back out a book" do
      it 'marks the book as checked out' do
        customer = create(:customer)
        book = create(:book)
        customer_book = create(:customer_book, customer_id: customer.id, bookable_id: book.id,
          status: "returned")

        request.env["HTTP_REFERER"] = list_books_path(customer_id: customer.id)
        post :check_out, id: book.id, customer_id: customer.id

        customer_book.reload

        expect(customer_book.status).to eql('checked out')
      end
      ...
      it 'displays a success flash message' do
        ...
        expect(flash[:success]).to eql("Book has been marked as checked out.")
      end

      it 'assigns customer' do
        customer = create(:customer)
        book = create(:book)
        customer_book = create(:customer_book, customer_id: customer.id, bookable_id: book.id, status: "returned")
  ...
  context "when the customer unsuccessfully checks back out a book" do
    it 'does not mark the book as checked out' do
      customer = create(:customer)
      book = create(:book)
      customer_book = create(:customer_book, customer_id: customer.id, bookable_id: book.id, status: "returned")
      ...
      expect(customer_book.status).to_not eql("checked out")
    end
    ...
    it 'displays the correct error flash message' do
      ...
      expect(flash[:error]).to eql('Book could not be marked as checked out.')
    end
  end
